...
- Secrets Manager: creates placeholders/secrets named `scavenger-hunt/GoogleOAuthCredentials` and `scavenger-hunt/AppleOAuthCredentials` (fill with real OAuth credentials post-deploy).
- SSM parameters exported for downstream config:
  - `/scavenger-hunt/auth/cognito-domain-prefix`
...
Phase 4 additions
-----------------
- `DataStack` now provisions DynamoDB tables for all domain entities: `Hunts`, `Tasks`, `Teams`, `TeamMemberships`, `JudgeAssignments`, `Submissions`, `TeamScores` (plus the existing `Users` table), with GSIs to list by owner/hunt/user/task/team.
- `CoreStack` wires every table name into Lambda environments and grants read/write access.
- Backend repositories for all entities (hunts, tasks, teams, memberships, judges, submissions, scores) use DynamoDB v3 DocumentClient and are covered by unit tests.
- New Hunts API Lambda (`/hunts`, `/hunts/{id}`) supports create/list/get/update for owners only; exposed via API Gateway with CORS enabled.
